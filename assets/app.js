(function(){
  const VERSION = '2025-08-29T10:20';

  // Etiquetas estáticas
  const I18N_LABELS={es:{brand:"Ángel Ortiz",nav:{home:"Inicio",summary:"Resumen",phases:"Fases",career:"Carrera",achievements:"Logros",skills:"Habilidades",awards:"Reconocimientos",charts:"Gráficos",contact:"Contacto"},sections:{summary:"Resumen Ejecutivo",phases:"Fases de Carrera",career:"Trayectoria",achievements:"Logros",skills:"Habilidades",awards:"Reconocimientos & Premios",charts:"Gráficos",contact:"Contacto"},charts:{yearsByPhase:"Años por Fase",skillsRadar:"Matriz de Competencias"},cta:{downloadCv:"Descargar CV (PDF)"},contact:{emailLabel:"Correo:"}},en:{brand:"Ángel Ortiz",nav:{home:"Home",summary:"Summary",phases:"Phases",career:"Career",achievements:"Achievements",skills:"Skills",awards:"Awards",charts:"Charts",contact:"Contact"},sections:{summary:"Executive Summary",phases:"Career Phases",career:"Career Timeline",achievements:"Key Achievements",skills:"Skills",awards:"Recognitions & Awards",charts:"Charts",contact:"Contact"},charts:{yearsByPhase:"Years by Phase",skillsRadar:"Skills Matrix"},cta:{downloadCv:"Download CV (PDF)"},contact:{emailLabel:"Email:"}}};
  ...
  function setStaticLabels(lang){
    ...
    const lbl=document.getElementById('langLabel'); if(lbl) lbl.textContent=(lang||'EN').toUpperCase();
  }
  ...
  // Render helpers (igual que la versión anterior)
  function fillHeader(cfg){
    const t=(id,val)=>{ const el=document.getElementById(id); if(el) el.textContent=val; };
    t('name',cfg.personal.name); t('title',cfg.personal.title); t('subtitle',cfg.personal.subtitle);
    const e1=document.getElementById('emailLink'), e2=document.getElementById('emailLink2');
    if(e1){ e1.textContent=cfg.personal.email; e1.href='mailto:'+cfg.personal.email; }
    if(e2){ e2.textContent=cfg.personal.email; e2.href='mailto:'+cfg.personal.email; }
  ...
  function renderAwards(cfg){
    ...
    (cfg.awards||[]).forEach(a=>{
      ...
      const title=a.title? `<h4 class="award-title">${(a.link||a.video)?`<a href="${(a.link||a.video)}" target="_blank" rel="noopener">${a.title}</a>`:a.title}</h4>` : '';
      let mediaHtml='';
      if(a.image){ const src=a.image.startsWith('http')?a.image:`img/${a.image}`; mediaHtml=`<img class="media" src="${src}" alt="${a.title||'award'}" loading="lazy">`; }
      else if(a.video){ const id=getYouTubeId(a.video); if(id){ const thumb=`https://img.youtube.com/vi/${id}/hqdefault.jpg`; mediaHtml=`<img class="media" src="${thumb}" alt="${a.title||'video'}" loading="lazy">`; } }
  ...
  function renderCharts(cfg){
    ...
    if(c1){ new Chart(c1,{type:'bar',data:{labels:cfg.career_years.labels,datasets:[{label:'Years',data:cfg.career_years.values,backgroundColor:['#1e3a8a','#3b82f6','#60a5fa']}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,ticks:{stepSize:2}}}}}); }
    const c2=document.getElementById('skillsRadar');
    if(c2){ new Chart(c2,{type:'radar',data:{labels:cfg.radar.labels,datasets:[{label:'Level',data:cfg.radar.values,borderColor:'#1e3a8a',backgroundColor:'rgba(30,58,138,0.2)',pointBackgroundColor:'#1e3a8a'}]},options:{responsive:true,maintainAspectRatio:false,scales:{r:{suggestedMin:0,suggestedMax:10,ticks:{stepSize:2}}}}}); }
  }
  ...
  async function applyLanguage(lang){
    try{
      ...
      fillHeader(cfg); renderSummary(cfg); renderPhases(cfg); renderLineTimeline(cfg);
      renderAchievements(cfg); renderSkills(cfg); renderAwards(cfg); renderCharts(cfg);
    }catch(err){
      console.error('Error cargando idioma', lang, err);
      if(lang!=='en'){ // fallback a EN
        try{ setStaticLabels('en'); const cfg = await loadConfig('en'); fillHeader(cfg); renderSummary(cfg); renderPhases(cfg); renderLineTimeline(cfg); renderAchievements(cfg); renderSkills(cfg); renderAwards(cfg); renderCharts(cfg); }catch(e2){ console.error('Fallback EN también falló', e2); }
      }
    }
  }
  ...
  document.addEventListener('DOMContentLoaded', async ()=>{
    ...
    initLangSelector(applyLanguage);
  });
})();
